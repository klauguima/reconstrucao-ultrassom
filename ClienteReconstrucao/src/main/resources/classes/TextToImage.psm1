class TextToImage {
	...
	[void] aprensentaImagemTeste() {
		[System.Collections.ArrayList]$tMatrix=@{}
        foreach($line in [System.IO.File]::ReadLines($this.hFilePath)) {
            $this.hMatrix.Add($line.split(",")) | Out-Null
			foreach($newLine in $this.hMatrix[0]) {
				[System.Collections.ArrayList]$arrLine=@{}
				$arrLine.Add([float]$newLine)
				$tMatrix.Add($arrLine)
			}
			
			break
        }
        $this.createImage($tMatrix)
	}
}
